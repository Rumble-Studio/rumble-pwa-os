 ...
 */
const sections: { [key: string]: { name: string; type: string } } = {
...
class WebmUint extends WebmBase {
	constructor(name: string, type: string = 'Uint') {
		super(name, type);
	}
...
class WebmFloat extends WebmBase {
	constructor(name: string, type: string = 'Float') {
		super(name, type);
	}
...
class WebmContainer extends WebmBase {
	...
	constructor(name: string, type: string = 'Container') {
		super(name, type);
	}
	...
	updateBySource() {
		...
		for (this.offset = 0; this.offset < this.source.length; this.offset = end) {
			...
			// console.log('[fix-webm-duration] type:', info.type);
			let ctr = WebmBase;
			switch (info.type) {
				case 'Container':
					ctr = WebmContainer;
...
class WebmFile extends WebmContainer {
	...
	toBlob(mimeType: string, filename = 'file'): File {
		return new File([this.source.buffer], filename, {
			type: mimeType || 'audio/webm',
		});
	}
}

export function fixWebmDuration(inputFile: File, duration: number): Promise<File> {
	...
	return new Promise(function (resolve) {
		try {
			...
			reader.onloadend = function () {
				let newFile: File;
				try {
					const file = new WebmFile(new Uint8Array(<ArrayBuffer>reader.result));
					if (file.fixDuration(duration)) {
						newFile = file.toBlob(inputFile.type, inputFile.name);
						resolve(newFile);
					}
				} catch (ex) {
					// ignore
					console.log('[fix-webm-duration] error found: returning original file', ex);
				}
				resolve(inputFile);
			};
