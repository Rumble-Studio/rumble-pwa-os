const webm =
	'data:video/webm;base64,';
const mp4 =
	'data:video/mp4;base64,';

// Detect native Wake Lock API support
const nativeWakeLock = () => 'wakeLock' in navigator;
// We need to do this to allow wakeLock property to be part of the navigator object
const navigatorProxy: any = navigator;
// We need to do this to allow MSStream property to be part of the window object
const windowProxy: any = window;

// Detect iOS browsers < version 10
const oldIOS = () =>
	typeof navigator !== 'undefined' &&
	parseFloat(
		('' + (/CPU.*OS ([0-9_]{3,4})[0-9_]{0,1}|(CPU like).*AppleWebKit.*Mobile/i.exec(navigator.userAgent) || [0, ''])[1])
			.replace('undefined', '3_2')
			.replace('_', '.')
			.replace('_', '')
	) < 10 &&
	!windowProxy.MSStream;

export class PreventDeviceToSleep {
	private _enabled?: boolean;
	private _wakeLock: any | null = null;
	private _preventDeviceToSleepTimer: number | null = null;
	private _preventDeviceToSleepVideo?: HTMLVideoElement;

	/**
	 * enable() to prevent device to go in sleep mode
	 *
	 * disable() to allow device to go in sleep mode
	 */
	constructor() {
		this._enabled = false;
		if (nativeWakeLock()) {
			this._wakeLock = null;
			const handleVisibilityChange = () => {
				if (this._wakeLock !== null && document.visibilityState === 'visible') {
					this.enable();
				}
			};
			document.addEventListener('visibilitychange', handleVisibilityChange);
			document.addEventListener('fullscreenchange', handleVisibilityChange);
		} else if (oldIOS()) {
			this._preventDeviceToSleepTimer = null;
		} else {
			// Set up no sleep video element
			this._preventDeviceToSleepVideo = document.createElement('video');

			this._preventDeviceToSleepVideo.setAttribute('title', 'No Sleep');
			this._preventDeviceToSleepVideo.setAttribute('playsinline', '');

			this._addSourceToVideo(this._preventDeviceToSleepVideo, 'webm', webm);
			this._addSourceToVideo(this._preventDeviceToSleepVideo, 'mp4', mp4);

			this._preventDeviceToSleepVideo.addEventListener('loadedmetadata', () => {
				if (!this._preventDeviceToSleepVideo) return;
				if (this._preventDeviceToSleepVideo.duration <= 1) {
					// webm source
					this._preventDeviceToSleepVideo.setAttribute('loop', '');
				} else {
					// mp4 source
					this._preventDeviceToSleepVideo.addEventListener('timeupdate', () => {
						if (!this._preventDeviceToSleepVideo) return;
						if (this._preventDeviceToSleepVideo.currentTime > 0.5) {
							this._preventDeviceToSleepVideo.currentTime = Math.random();
						}
					});
				}
			});
		}
	}

	private _addSourceToVideo(element: HTMLVideoElement, type: string, dataURI: string) {
		const source = document.createElement('source');
		source.src = dataURI;
		source.type = `video/${type}`;
		element.appendChild(source);
	}

	get isEnabled() {
		return this._enabled;
	}

	enable() {
		if (nativeWakeLock()) {
			return navigatorProxy['wakeLock']
				.request('screen')
				.then((wakeLock: any) => {
					this._wakeLock = wakeLock;
					this._enabled = true;
				})
				.catch((err: any) => {
					this._enabled = false;
					console.error(`${err.name}, ${err.message}`);
					throw err;
				});
		} else if (oldIOS()) {
			this.disable();
			console.warn(`
        PreventDeviceToSleep enabled for older iOS devices. This can interrupt
        active or long-running network requests from completing successfully.
        See https://github.com/richtr/NoSleep.js/issues/15 for more details.
      `);
			this._preventDeviceToSleepTimer = window.setInterval(() => {
				if (!document.hidden) {
					window.location.href = window.location.href.split('#')[0];
					window.setTimeout(window.stop, 0);
				}
			}, 15000);
			this._enabled = true;
			return Promise.resolve();
		} else {
			if (!this._preventDeviceToSleepVideo) return;
			const playPromise = this._preventDeviceToSleepVideo.play();
			return playPromise
				.then((res) => {
					this._enabled = true;
					return res;
				})
				.catch((err) => {
					this._enabled = false;
					throw err;
				});
		}
	}

	disable() {
		if (nativeWakeLock()) {
			if (this._wakeLock) {
				this._wakeLock.release();
			}
			this._wakeLock = null;
		} else if (oldIOS()) {
			if (this._preventDeviceToSleepTimer) {
				console.warn(`
          NoSleep now disabled for older iOS devices.
        `);
				window.clearInterval(this._preventDeviceToSleepTimer);
				this._preventDeviceToSleepTimer = null;
			}
		} else {
			this._preventDeviceToSleepVideo?.pause();
		}
		this._enabled = false;
	}
}
