...
export class ResizableDirective implements OnInit, OnDestroy {
	...
	ngOnInit(): void {
		...
		const mousemove$ = merge(this.pointerEventListeners.pointerMove, this.mousemove).pipe(
			// tap(({ event }) => {
			// 	if (currentResize) {
			// 		try {
			...
			.pipe(
				mergeMap((startCoords) => {
					...
					return (
						...
						) as Observable<[{ clientX: number; clientY: number }, { clientX: number; clientY: number }]>
					)
						.pipe(
		...
		mousedown$
			...
			.subscribe((edges: Edges) => {
				...
				if (this.enableGhostResize) {
					...
					currentResize.clonedNode!.scrollTop = currentResize.startingRect.scrollTop as number;
					currentResize.clonedNode!.scrollLeft = currentResize.startingRect.scrollLeft as number;
				}
		...
		mouseup$.pipe(takeUntil(this.destroy$)).subscribe(() => {
			if (currentResize) {
				...
				this.renderer.setStyle(this.elm.nativeElement, 'width', `${currentResize.currentRect.width}px`);
				this.renderer.setStyle(this.elm.nativeElement, 'height', `${currentResize.currentRect.height}px`);
				this.renderer.setStyle(this.elm.nativeElement, 'top', `${currentResize.currentRect.top}px`);
				this.renderer.setStyle(this.elm.nativeElement, 'left', `${currentResize.currentRect.left}px`);
